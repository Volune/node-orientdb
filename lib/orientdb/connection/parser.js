...
var readRecord = function(buf, objOffset) {
    ...
    if (record["class"] === -2) {
        ...
        offset += BYTES_INT + (readStringContent.lengthInBytes > 0 ? readStringContent.lengthInBytes : 0);
    } else {
...
/*------------------------------------------------------------------------------
 ...
 Parse data to 1 byte which represents boolean value. True if and only if byte value is 1.
 ------------------------------------------------------------------------------*/
var writeBoolean = function(data) {
    var buf = new Buffer(BYTES_BOOLEAN);
...
var canReadError = function(buf, offset) {
    ...
    while (more) {
        ...
        offset += BYTES_INT + (result.lengthInBytes > 0 ? result.lengthInBytes : 0);
...
var readError = function(buf, objOffset) {
    ...
    while (more) {
        ...
        offset += BYTES_INT + (errorClass.lengthInBytes > 0 ? errorClass.lengthInBytes : 0);

        // exception class
        var errorMessage = readString(buf, offset);
        error.message = errorMessage.value;
        offset += BYTES_INT + (errorMessage.lengthInBytes > 0 ? errorMessage.lengthInBytes : 0);
