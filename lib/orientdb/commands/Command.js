...
command.write = function (socket, sessionId, data, callback) {
    ...
    //for aliases, see com.orientechnologies.orient.core.serialization.serializer.stream.OStreamSerializerAnyStreamable.fromStream(..)
    switch (data.className) {
        ...
        case 'c':
        case 'C':
        case 'com.orientechnologies.orient.core.sql.OCommandSQL':
        default:
            writeOCommandRequestTextAbstract(data, buffers);
    }

    // append all buffers into one
    var i, size, length = buffers.length;
...
function writeOCommandRequestTextAbstract(data, buffers) {
    ...
    var document;
    var simpleParams = data.params && !_.isEmpty(data.params);
    buffers.push(parser.writeBoolean(simpleParams));
    if (simpleParams) {
        document = {params: data.params};
        //warning: orientDb deserialize the document using getAsByteArray, not getAsString.
        //writing with parser.writeString should give a compatible result, but there were no strong test about that.
        buffers.push(parser.writeString(parser.serializeDocument(document)));
    }

    // TODO serialized composite key params
    var compositeKeyParamsPresent = false;
    buffers.push(parser.writeBoolean(compositeKeyParamsPresent));
}
...
function writeOSQLQuery(data, buffers) {
    ...
    var serializedParamsDocument;
    if (data.params && !_.isEmpty(data.params)) {
        var paramsDocument = {params: data.params};
        serializedParamsDocument = parser.serializeDocument(paramsDocument);
    }
    else
        serializedParamsDocument = '';
    buffers.push(parser.writeString(serializedParamsDocument));
}
